...
if [ -d "${CACHE_ROOT}/${TARGETARCH}/mediasoup/out" ]; then
  ...
  mv ${CACHE_ROOT}/${TARGETARCH}/mediasoup/out ${WORK_ROOT}/src/lib/ext/mediasoup/worker
fi
if [ -d "${CACHE_ROOT}/${TARGETARCH}/mediasoup/pip_invoke" ]; then
  echo "Using cached mediasoup build tools"
  mv ${CACHE_ROOT}/${TARGETARCH}/mediasoup/pip_invoke ${WORK_ROOT}/src/lib/ext/mediasoup/worker
fi
...
if [ -d "${WORK_ROOT}/src/lib/ext/mediasoup/worker/out" ]; then
  echo "Cache mediasoup artifacts"
  mv ${WORK_ROOT}/src/lib/ext/mediasoup/worker/out ${CACHE_ROOT}/${TARGETARCH}/mediasoup
fi
if [ -d "${WORK_ROOT}/src/lib/ext/mediasoup/worker/pip_invoke" ]; then
  echo "Cache mediasoup build tools"
  mv ${WORK_ROOT}/src/lib/ext/mediasoup/worker/pip_invoke ${CACHE_ROOT}/${TARGETARCH}/mediasoup
fi
