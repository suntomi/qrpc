...
namespace qrpc {
void Boxer::Processor::Poll(Boxer *p) {
  Op *op;
  while (try_dequeue(op)) {
    ...
    case Conn: {
      ...
      auto m = Unwrapper::UnsafeUnwrapMutex(ConnSerialCodec::IsClient(op->serial_), c);
      ...
      case Reconnect:
      case Disconnect:
        p->InvokeConn(op->serial_, c, op->code_, op->disconnect_.reason, 
                      reinterpret_cast<const uint8_t *>(op->data_.ptr()), op->data_.length(), true);
        break;
      case OpenStream:
...
#if defined(USE_DIRECT_WRITE)
      auto c = reinterpret_cast<NqSessionDelegate *>(op->target_ptr_);
      auto m = Unwrapper::UnsafeUnwrapMutex(StreamSerialCodec::IsClient(op->serial_), c);
      std::unique_lock<std::mutex> lock(*m);
      p->LockSession(c->SessionIndex());
#endif
