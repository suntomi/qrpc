...
export class QRPClient {
    ...
    async #syscallMessageHandler(s, event) {
        ...
        if (_a.VERBOSE_SYSCALL.indexOf(data.fn) < 0) {
        ...
        else if (data.fn == "resume_ack" || data.fn == "pause_ack" || data.fn == "close_ack" ||
    ...
    #newMsgId() {
        if (this.msgidSeed >= _a.MAX_MSGID) {
            this.msgidSeed = 1;
        }
    ...
    parseLocalOffer(localOffer) {
        ...
        return Object.fromEntries(Object.entries(result).filter(v => Object.keys(v[1]).length == 1).map(v => [v[0], Object.keys(v[1])[0]]));
    }
    #checkMedias(now) {
        ...
        for (const k in this.medias) {
            ...
            if (m.isReceiver) {
                ...
                if (now - m.lastPing > (_a.NO_INPUT_THRESHOLD * 1000)) {
                    if (m.opened) {
                        m.opened = false;
                        const reconnectionWaitMS = m.pause(QRPCTrack.PAUSE_REASON.remote_close);
                        console.log(`no ping for ${m.path} for ${_a.NO_INPUT_THRESHOLD * 1000} ms`, reconnectionWaitMS);
    ...
    async #setRemoteOffer(remoteOffer, sdpGen, sentTracks) {
        ...
        if (this.#handshaked()) {
            ...
            await this.syscall("remote_answer", {
                midMap: Object.fromEntries(Object.keys(midSsrcMap).map(k => [k, {
    ...
    async #handshake() {
        ...
        const syscallReady = new Promise((resolve, reject) => {
            this.syscallStream = this.openStream(_a.SYSCALL_STREAM, {
    ...
    async openMedia(path, params) {
        ...
        if (!encodings) {
            throw new Error("encodings is mandatory");
        }
        if (encodings.length > 3) {
            throw new Error("encodings more than 3 may not be treated correctly");
        }
        const ridScalabilityModeMap = {};
        let index = 0;
        encodings.forEach(e => {
            if (!e.maxBitrate) {
                throw new Error("for each encodings, maxBitrate is mandatory");
            }
            if (e.rid) {
                throw new Error(`cannot specify rid for encodings`);
            }
            e.rid = `r${index++}`;
            e.scalabilityMode = e.scalabilityMode || _a.DEFAULT_SCALABILITY_MODE;
    ...
    #addMedia(path, direction, initOptions) {
    ...
    async watchMedia(path, params) {
        ...
        for (const k of (kind ? [kind] : ["video", "audio"])) {
            ...
            if (!track) {
                if (initOptions?.sync) {
                    throw new Error(`no track for ${path} yet but sync option is set`);
                }
    ...
    async updateMedia(path, options) {
    ...
    #setupStream(s, h) {
        ...
        s.onopen = (h.onopen && (async (event) => {
            if (h.publish) {
                await this.syscall("publish_stream", { path });
            }
