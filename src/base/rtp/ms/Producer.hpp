...
namespace ms {
	...
	public:
		...
		public:
			virtual void OnProducerReceiveData(ms::Producer* producer, size_t len)                  = 0;
			virtual void OnProducerReceiveRtpPacket(ms::Producer* producer, RTC::RtpPacket* packet) = 0;
			virtual void OnProducerPaused(ms::Producer* producer)                                   = 0;
			virtual void OnProducerResumed(ms::Producer* producer)                                  = 0;
			virtual void OnProducerNewRtpStream(
			  ms::Producer* producer, RTC::RtpStreamRecv* rtpStream, uint32_t mappedSsrc) = 0;
			...
			virtual void OnProducerRtcpSenderReport(
			  ms::Producer* producer, RTC::RtpStreamRecv* rtpStream, bool first)                     = 0;
			virtual void OnProducerRtpPacketReceived(ms::Producer* producer, RTC::RtpPacket* packet) = 0;
			virtual void OnProducerSendRtcpPacket(ms::Producer* producer, RTC::RTCP::Packet* packet) = 0;
			virtual void OnProducerNeedWorstRemoteFractionLost(
			  ms::Producer* producer, uint32_t mappedSsrc, uint8_t& worstRemoteFractionLost) = 0;
		};
	...
	public:
		Producer(ms::Shared* shared, const std::string& id, ms::Producer::Listener* listener, json& data);
		virtual ~Producer();
	...
	private:
		// Passed by argument.
		ms::Shared* shared{ nullptr };
		ms::Producer::Listener* listener{ nullptr };
		// Allocated by this.
