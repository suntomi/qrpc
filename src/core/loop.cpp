...
namespace qrpc {
// handling alarm
void Loop::SetAlarm(AlarmInterface *a, uint64_t timeout_in_us) {
  alarm_map_.emplace(std::piecewise_construct, 
                    std::forward_as_tuple(timeout_in_us), 
                    std::forward_as_tuple(a));
}
void Loop::CancelAlarm(AlarmInterface *a, uint64_t timeout_in_us) {
...
void Loop::Poll() {
  ...
  while (true) {
    ...
    AlarmInterface* cb = static_cast<NqAlarmInterface*>(it->second.ptr_);
